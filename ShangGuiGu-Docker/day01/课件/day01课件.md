...
- [1 docker初体验](#1-docker初体验)
  - [1.1 docker简介](#11-docker简介)
  - [1.2 docker的优势](#12-docker的优势)
  - [1.3 docker和虚拟机的区别](#13-docker和虚拟机的区别)
  - [1.4 docker架构](#14-docker架构)
  - [1.5 docker的安装与卸载](#15-docker的安装与卸载)
  - [1.6 docker服务相关命令](#16-docker服务相关命令)
  - [1.7 阿里云镜像加速器](#17-阿里云镜像加速器)
- [2 docker镜像操作](#2-docker镜像操作)
  - [2.1 搜索远程镜像](#21-搜索远程镜像)
  - [2.2 拉取镜像](#22-拉取镜像)
  - [2.3 查看本地镜像](#23-查看本地镜像)
  - [2.4 删除本地镜像](#24-删除本地镜像)
  - [2.5 帮助文档使用](#25-帮助文档使用)
- [3 docker容器操作](#3-docker容器操作)
  - [3.1 查询容器](#31-查询容器)
  - [3.2 创建容器](#32-创建容器)
    - [3.2.1 容器创建入门  run命令](#321-容器创建入门--run命令)
    - [3.2.2 常见参数选项](#322-常见参数选项)
      - [--name](#--name)
      - [-p和Dockerfile中port的作用](#-p和dockerfile中port的作用)
  - [3.3 容器服务管理](#33-容器服务管理)
  - [3.4 删除容器](#34-删除容器)
  - [3.5 进入容器](#35-进入容器)
  - [3.6 其他命令](#36-其他命令)
  - [3.7 备份与迁移](#37-备份与迁移)
- [4 docker数据卷操作](#4-docker数据卷操作)
  - [4.1 数据卷概述](#41-数据卷概述)
  - [4.2 常见命令](#42-常见命令)
    - [4.2.1 查看数据卷](#421-查看数据卷)
    - [4.2.2 创建数据卷](#422-创建数据卷)
    - [4.2.3 查询数据卷详情](#423-查询数据卷详情)
    - [4.2.4 删除数据卷](#424-删除数据卷)
  - [4.3 数据卷挂载](#43-数据卷挂载)
  - [4.4 目录挂载](#44-目录挂载)
- [5 Portainer工具](#5-portainer工具)
  - [5.1 安装与启动](#51-安装与启动)
  - [5.2 镜像管理](#52-镜像管理)
  - [5.3 容器管理](#53-容器管理)
- [6 Spring Boot项目部署](#6-spring-boot项目部署)
  - [6.1 dockerfile](#61-dockerfile)
    - [6.1.1 dockerfile简介](#611-dockerfile简介)
    - [6.1.2 入门案例](#612-入门案例)
  - [6.2 案例介绍与需求分析](#62-案例介绍与需求分析)
  - [6.3 docker部署Mysql](#63-docker部署mysql)
  - [6.4 dockerfile构建镜像](#64-dockerfile构建镜像)
    - [6.4.1 项目打包](#641-项目打包)
    - [6.4.2 dockerfile文件](#642-dockerfile文件)
    - [6.4.3 构建镜像](#643-构建镜像)
  - [6.5 创建容器](#65-创建容器)
- [7 Maven的docker插件](#7-maven的docker插件)
  - [大致整体流程](#大致整体流程)
  - [插件配置步骤：](#插件配置步骤)
  - [实操：](#实操)
    - [1、在pom.xml文件中添加Maven的docker插件](#1在pomxml文件中添加maven的docker插件)
    - [2、docker服务端开启远程访问---\>方便跨机器管理 Docker 资源](#2docker服务端开启远程访问---方便跨机器管理-docker-资源)
    - [3、编写dockerfile文件](#3编写dockerfile文件)
  - [注意：](#注意)
- [8 docker compose(了解)](#8-docker-compose了解)
  - [8.1 docker compose简介](#81-docker-compose简介)
  - [8.2 下载与安装](#82-下载与安装)
  - [8.3 入门案例](#83-入门案例)
  - [8.4 编排Spring Boot项目](#84-编排spring-boot项目)
- [9 docker镜像仓库](#9-docker镜像仓库)
  - [9.1 镜像仓库简介](#91-镜像仓库简介)
  - [9.2 Harbor简介](#92-harbor简介)
  - [9.3 下载与安装](#93-下载与安装)
    - [9.3.1 下载](#931-下载)
    - [9.3.2 安装](#932-安装)
  - [9.4 Harbor使用](#94-harbor使用)
    - [9.4.1 上传镜像](#941-上传镜像)
    - [9.4.2 拉取镜像](#942-拉取镜像)
...
**docker主机**：docker主机就是docker的服务端、在docker主机中存在一个守护进程。这个守护进行负责管理docker中常见的一些对象（比如：镜
...
#### -p和Dockerfile中port的作用
1. 容器默认不开放任何端口号,在Dockerfile配置文件中的port字段是说让容器开放哪个端口号
   1. 容器内的应用程序监听的端口号应与容器开放端口号保持一致
   2. 若不一致也会访问失败的,映射不到容器内的进程
2. 当容器开放了端口号,宿主机上默认还不能访问:
   1. docker容器内部所运行的进程是无法被外部机器(windows)直接访问的, 如果外部机器向访问容器内的进程，那么在创建容器的时候就需要在linux宿主机上开一个端口号，并且需要建立这个端口号和容器内进程端口号之间的映射关系，这样外部机器就可以通过宿主机的端口号访问到容器内的进程了
...
# 点 表示Dockerfile所在的路径（这里的.代表当前目录）。(Docker 会读取该目录下的Dockerfile文件，并根据其中的指令（如FROM、COPY、RUN等）构建镜像)
# -t 表示镜像的名称
...
刚才的构建过程是手动镜像构建的过程。使用Maven的docker插件可以实现镜像的自动化部署。省去了本地构建镜像再手动上传到服务器里的步骤

## 大致整体流程

1. 本地执行`mvn package`后,本地生成target/项目名.jar包文件
2. maven-docker插件读取Dockfile文件
3. 插件将Dockerfile和生成的包文件 打包成tar,自动发送到远端docker服务器(192.168.136.142:2375)
   1. 2375端口号:是docker服务的一个隐藏端口号,专门用来接收远程命令的
4. 远端接收后执行docker build 生成镜像,存储在远程服务器
   1. 本地文件不会被 “持久化” 到远程服务器的某个固定目录，而是以 临时 tar 包 的形式传输给远程 Docker，用于构建镜像
5. 本地终端显示BUILD SUCCESS,远程构建完成,镜像存储在远程服务器
...
<plugin>
    ...
    <configuration>
        ...
        <!-- 2375端口号:是docker服务的一个隐藏端口号,专门用来接收远程命令的 -->
        <dockerHost>http://192.168.136.142:2375</dockerHost>

        <!-- 指定 dockerfile 路径(basedir指当前项目的根目录路径（即 pom.xml 文件所在的目录)-->
...
### 2、docker服务端开启远程访问--->方便跨机器管理 Docker 资源

1. 默认情况下，Docker 守护进程只监听本地的 unix:///var/run/docker.sock 套接字，只允许本地机器通过 docker 命令操作，远程机器无法直接访问
   1. 通过配置: --- 配置 Docker 守护进程（dockerd）允许远程访问，通过修改 Docker 的系统服务配置，让 Docker 服务可以通过网络（TCP 协议）被远程机器访问和控制。
--------
2. 注意事项:
   1. 通常用于内网环境（如公司内部服务器之间的 Docker 通信），不建议在公网环境直接使用此配置
   2. 生产环境建议：使用 2375 端口并配置 TLS 加密（需生成证书），或通过防火墙限制仅允许特定 IP 访问
      1. 直接暴露在公网会导致严重安全隐患（任何人都可远程控制你的 Docker，甚至获取服务器 root 权限

```shell
#修改该文件 ----- Docker 服务在系统中的配置文件,定义 Docker 服务的启动参数、依赖关系等
vim /lib/systemd/system/docker.service

#找到ExecStart行，修改成如下内容
# ExecStart 用于指定 Docker 服务启动时执行的命令
ExecStart=/usr/bin/dockerd -H tcp://0.0.0.0:2375 -H fd:// --containerd=/run/containerd/containerd.sock
# -H :指定 Docker 守护进程监听的地址和端口,远程机器可通过 IP:2375 访问
# -H fd:// :指定 Docker 守护进程监听的 Unix 域套接字文件描述符,用于与 Docker 客户端进行通信,确保本地 Docker 命令正常执行
...
1. 本地执行完打包命令,会自动链接远程docker,将包和Dockefile发到远程构建出一个镜像,保存到远程服务器,并返回镜像的id
2. 此时在远程服务器用docker images命令可以看到新创建了个镜像
...
前面我们是直接创建镜像到linux机器中保存,这样危险,万一linux挂了,镜像就丢失了,所以我们需要将镜像保存到云上镜像仓库中,这样即使linux挂了,镜像仓库中的镜像还在,我们可以从镜像仓库中拉取镜像,重新创建容器
...
它以Docker公司开源的registry为基础，提供了管理UI，基于角色的访问控制(Role Based Access Control)，AD/LDAP集成、以及审计日志(Auditlogging) 等企业

用户需求的功能，同时还原生支持中文。

以官方registny 为基础，加入了mysql, reds, nginx 等其他8个组件创建了一个可以界面化操作的私服应用, 将源代码上传到github，源代码中使用的都是docker公共仓库中的 mySql，redis，nginx
